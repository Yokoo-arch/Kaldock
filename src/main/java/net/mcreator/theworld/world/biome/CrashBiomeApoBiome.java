...
public class CrashBiomeApoBiome extends TheWorldModElements.ModElement {

	@ObjectHolder("the_world:crash_biome_apo")
	public static final CustomBiome biome = null;

	public CrashBiomeApoBiome(TheWorldModElements instance) {
		super(instance, 83);
	}
	...
	static class CustomBiome extends Biome {
		...
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(ToxicGrassItem.block.getDefaultState(),
							Blocks.STONE.getDefaultState(), Blocks.STONE.getDefaultState())));

			setRegistryName("crash_biome_apo");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(ToxicLogItem.block.getDefaultState()),
									new SimpleBlockStateProvider(ToxicLeavesItem.block.getDefaultState()))).baseHeight(4)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		@Override protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks, Set<BlockPos> changedBlocks2, MutableBoundingBox bbox, BaseTreeFeatureConfig conf) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == ToxicGrassItem.block.getDefaultState().getBlock()
							|| ground == Blocks.STONE.getDefaultState().getBlock())
							&& (ground2 == ToxicGrassItem.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos)
												...
												|| state.getBlock() == .getDefaultState().getBlock()
												|| state.getBlock() == ToxicLeavesItem.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world,
													blockpos, ToxicLeavesItem.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, ToxicLogItem.block.getDefaultState(), bbox);
		...
		private void addVines(IWorld world, BlockPos pos, Set<BlockPos> changedBlocks, MutableBoundingBox bbox) {
			setTreeBlockState(changedBlocks, world, pos, .getDefaultState(), bbox);
			int i = 5;
			for (BlockPos blockpos = pos.down(); world.isAirBlock(blockpos) && i > 0; --i) {
				setTreeBlockState(changedBlocks, world, blockpos, .getDefaultState(), bbox);
				blockpos = blockpos.down();
			}
		}

		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == ToxicLogItem.block.getDefaultState().getBlock()
					|| blockType == ToxicLeavesItem.block.getDefaultState().getBlock()
					|| blockType == ToxicGrassItem.block.getDefaultState().getBlock() || blockType == Blocks.STONE.getDefaultState().getBlock();
		}
