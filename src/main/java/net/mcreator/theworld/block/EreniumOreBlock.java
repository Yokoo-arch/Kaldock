...
public class EreniumOreBlock extends TheWorldModElements.ModElement {
	@ObjectHolder("the_world:erenium_ore")
	public static final Block block = null;
	public EreniumOreBlock(TheWorldModElements instance) {
		super(instance, 2);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("erenium_ore", "erenium_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 16)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(10, 1, 1, 22))));
		}
	}
}
