...
import net.mcreator.theworld.block.ToxicLogBlock;
import net.mcreator.theworld.block.ToxicLeavesBlock;
import net.mcreator.theworld.block.ToxicGrassBlock;
...
public class CrashBiomeApoBiome extends TheWorldModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(ToxicGrassBlock.block.getDefaultState(),
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(ToxicLogBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(ToxicLeavesBlock.block.getDefaultState()))).baseHeight(4)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		@Override protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks, Set<BlockPos> changedBlocks2, MutableBoundingBox bbox, BaseTreeFeatureConfig conf) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == ToxicGrassBlock.block.getDefaultState().getBlock()
							|| ground == Blocks.STONE.getDefaultState().getBlock())
							&& (ground2 == ToxicGrassBlock.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos)
												...
												|| state.getBlock() == ToxicLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world,
													blockpos, ToxicLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, ToxicLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == ToxicLogBlock.block.getDefaultState().getBlock()
					|| blockType == ToxicLeavesBlock.block.getDefaultState().getBlock()
					|| blockType == ToxicGrassBlock.block.getDefaultState().getBlock() || blockType == Blocks.STONE.getDefaultState().getBlock();
		}
